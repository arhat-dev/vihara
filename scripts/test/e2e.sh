...
e2e() {
  ...
  mkdir -p build/e2e/charts/vihara

  cp -r cicd/deploy/charts/vihara \
    build/e2e/charts/vihara/master

  helm-stack -c e2e/helm-stack.yaml ensure

  # basic override for image pull secrets
  cp e2e/values/vihara.yaml "build/e2e/clusters/${kube_version}/vihara_master.default_vihara.yaml"
