...
vihara() {
  ...
  _build "CGO_ENABLED=0 ${GOBUILD} -tags='foo bar ${PREDEFINED_BUILD_TAGS}' ./cmd/vihara"
}
...
GO_LDFLAGS="-s -w \
  -X arhat.dev/vihara/pkg/version.branch=${GIT_BRANCH} \
  -X arhat.dev/vihara/pkg/version.commit=${GIT_COMMIT} \
  -X arhat.dev/vihara/pkg/version.tag=${GIT_TAG} \
  -X arhat.dev/vihara/pkg/version.arch=${ARCH} \
  -X arhat.dev/vihara/pkg/version.goCompilerPlatform=$(go version | cut -d\  -f4)"
